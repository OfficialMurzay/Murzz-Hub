...
-------------------------------------------------
local ClassMap = {
    [1]="Fighter 🥊",[2]="Shinobi 🌀",[3]="Pirate ☠️",[4]="Ghoul 👻",[5]="Hero 🦸",
    [6]="Reaper ☠️",[7]="Saiyan 💥",[8]="Sin 😈",[9]="Magi 🧙",[10]="Akuma 😡",
    [11]="Yonko 🏴‍☠️",[12]="Gorosei 🧓",[13]="Overlord 👑",[14]="Hokage 🦊",
    [15]="Kaioshin 👼",[16]="Sage 🕊️",[17]="Espada ⚔️",[18]="Shinigami 👹",
    [19]="Hashira ⚔️",[20]="Hakaishin 💀",[21]="Otsutsuki 🌌",
    [22]="Pirate King 🏴‍☠️👑",[23]="Kishin 👿",[24]="Angel 😇",
    [25]="Demon King 👹",[26]="Ultra Instinct ✨",[27]="Upper Moon 🌙"
}

local StatMap = {
    ["1"]={"Strength","💪"},["2"]={"Durability","🛡️"},["3"]={"Chakra","🌀"},
    ["4"]={"Sword","🗡️"},["5"]={"Agility","☁️"},["6"]={"Speed","💨"},["Kill"]={"Kill","🔪"},
}
...
HomeTab:CreateSection("Player Data 👤")
...
bindValue(OtherData:FindFirstChild("TotalPower"), PowerLabel, "Total Power: ", "⚡")
bindValue(OtherData:FindFirstChild("Yen"), YenLabel, "Money: ", "💰")
bindValue(OtherData:FindFirstChild("Chikara"), ChikaraLabel, "Chikara: ", "💎")
...
local OrderedStats = {
    {id="1", name="Strength", emoji="💪"},
    {id="2", name="Durability", emoji="🛡️"},
    {id="3", name="Chakra", emoji="🌀"},
    {id="4", name="Sword", emoji="⚔️"},
    {id="6", name="Speed", emoji="💨"},
    {id="5", name="Agility", emoji="☁️"},
}
...
local function setupQuest(quest)
    safeRun(function()
        ...
        QuestsTab:CreateSection(display.." 📜")
        ...
        for _,req in ipairs(reqs:GetChildren()) do
            if req:IsA("IntValue") or req:IsA("NumberValue") then
                ...
                local function update()
                    ...
                    if StatMap[req.Name] then name, emoji = unpack(StatMap[req.Name]) end
                    ...
                    task.spawn(function()
                        ...
                        for i = 1, steps do
                            ...
                            label:Set(string.format("%s/%s %s %s%s",
                                abbreviate(math.floor(val)), abbreviate(req.Value), name, emoji, done and " ✅" or ""))
                            task.wait(duration/steps)
                        end
                        label:Set(string.format("%s/%s %s %s%s",
                            abbreviate(progress), abbreviate(req.Value), name, emoji, done and " ✅" or ""))
                        current = progress
                    end)
                end
...
-- AUTO FARMS TAB
-------------------------------------------------
local AutoTab = Window:CreateTab("Auto Farms 🌾")

-- Auto Train Section
AutoTab:CreateSection("Auto Train 🏋️")
...
AutoTab:CreateSection("Auto Chikara 💎 | AUTOCLICKER NEEDED")
local AutoChikaraActive = false

AutoTab:CreateToggle({
    Name = "Collect Chikara ⚠️",
    CurrentValue = false,
...
-- Stats Data
local StatsData = {
    {Name="Strength", SectionEmoji="💪", ButtonEmoji="📁", StatId="1", Zones={
    ...
    {Name="Durability", SectionEmoji="🛡️", ButtonEmoji="📁", StatId="2", Zones={
    ...
    {Name="Chakra", SectionEmoji="🌀", ButtonEmoji="📁", StatId="3", Zones={
    ...
    {Name="Agility", SectionEmoji="☁️", ButtonEmoji="📁", StatId="5", Zones={
    ...
    {Name="Speed", SectionEmoji="💨", ButtonEmoji="📁", StatId="6", Zones={
...
TeleportsTab:CreateSection("Best Stat Zones 📊")
...
SettingsTab:CreateLabel("None for now, srry!")
]]
...
local _e = {}
for i = 1, #_SRC do
    _e[i] = string.char((string.byte(_SRC, i) ~ 73) & 255)
end

local _payload = table.concat(_e)
_SRC = nil

local ok, err = pcall(function()
    loadstring(_d(_payload))()
end)

if not ok then
    warn("[MurzzHub] Runtime error:", err)
end
